...
const NEW_STRINGS = [
  "batches need immediate attention!",
  "OHI is",
  "— only",
  "At current market:",
  "— sell now to lock in this price.",
  "active batches"
];
...
async function injectFragments() {
  ...
  for (const lang of TARGET_LANGS) {
    ...
    console.log(`Translating fragments for ${lang.name}...`);
    try {
      const prompt = `Translate exactly these English sentence fragments into ${lang.name} for an agricultural app. 
      Return STRICTLY a JSON object mapping English to the translated string.
      Be sure to include any dashes or punctuation that exist in the English string!
  ...
  console.log('Fragments injected.');
}
injectFragments();
